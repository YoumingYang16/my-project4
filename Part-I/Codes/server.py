...
def get_file_md5(filename):
    ...
    """
    m = hashlib.md5()
    with open(filename, 'rb') as fid: # change 2
        while True:
            d = fid.read(2048)
            if not d:
                break
...
def data_process(username, request_operation, json_data, connection_socket):
    ...
    if request_operation == OP_SAVE: # change 4
        key = str(uuid.uuid4())
        ...
        try:
            with open(join('data', username, key), 'w') as fid:
                ...
                logger.info(f'<-- Data is saved with key "{key}"') # change 5
                connection_socket.send(
    ...
    if request_operation == OP_DELETE:
        ...
        try:
            ...
            logger.info(f'<-- The "key" {json_data[FIELD_KEY]} is deleted.') # change 5
            connection_socket.send(
...
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_GET:
        if FIELD_KEY not in json_data.keys():
            logger.info('--> Plan to download file without key') # change 14
            connection_socket.send(
        ...
        logger.info(f'<-- Plan: file size {file_size}, total block number {total_block}.') # change 6
        connection_socket.send(
    ...
    if request_operation == OP_SAVE:
        ...
        try:
            ...
            with open(join('tmp', username, key + '.log'), 'w') as fid:  # change 8
                pass                                                     # change 8

            logger.info(f'<-- Upload plan: key {key}, total block number {total_block}, block size {block_size}.') # change 5
            connection_socket.send(
    ...
    if request_operation == OP_DELETE:
        if FIELD_KEY not in json_data.keys():
            ...
            connection_socket.send(
                make_response_packet(OP_DELETE, 410, TYPE_FILE, f'Field "key" is missing for FILE delete.', {})) # change 7
            return
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is False:
            if os.path.exists(join('tmp', username, json_data[FIELD_KEY])) is True:
                ...
                connection_socket.send(
                    make_response_packet(OP_DELETE, 404, TYPE_FILE, # change 7
                                         f'The "key" {json_data[FIELD_KEY]} is not completely uploaded. '
            ...
            logger.error(f'<-- The "key" {json_data[FIELD_KEY]} is not existing.')
            connection_socket.send(
                make_response_packet(OP_DELETE, 404, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is not existing.', {})) # change 7
            return
        try:
            os.remove(join('file', username, json_data[FIELD_KEY]))
            logger.info(f'<-- The "key" {json_data[FIELD_KEY]} is deleted.') # change 5
            connection_socket.send(
                make_response_packet(OP_DELETE, 200, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is deleted.', # change 7
                                     {FIELD_KEY: json_data[FIELD_KEY]}))
        except Exception as ex:
            logger.error(f'{str(ex)}@{ex.__traceback__.tb_lineno}')
    ...
    if request_operation == OP_UPLOAD:
        ...
        with open(file_path + '.log', 'r') as fid: # change 8
            lines = fid.readlines()                # change 8
    ...
    if request_operation == OP_DOWNLOAD:
        if FIELD_KEY not in json_data.keys():
            ...
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 410, TYPE_FILE, f'Field "key" is missing for FILE downloading.', {})) # change 9
            return
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is False:
            if os.path.exists(join('tmp', username, json_data[FIELD_KEY])) is True:
                ...
                connection_socket.send(
                    make_response_packet(OP_DOWNLOAD, 404, TYPE_FILE, # change 9
                                         f'The "key" {json_data[FIELD_KEY]} is not completely uploaded. '
            ...
            logger.error(f'<-- The "key" {json_data[FIELD_KEY]} is not existing.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 404, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is not existing.', {})) # change 9
            return

        if FIELD_BLOCK_INDEX not in json_data.keys():
            logger.error(f'<-- The "block_index" is compulsory.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 410, TYPE_FILE, f'The "block_index" is compulsory.', {})) # change 9
            return
        file_path = join('file', username, json_data[FIELD_KEY])
        file_size = getsize(file_path)
        block_size = MAX_PACKET_SIZE
        ...
        if block_index >= total_block:
            logger.error(f'<-- The "block_index" exceed the max index.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 410, TYPE_FILE, f'The "block_index" exceed the max index.', {})) # change 9
            return
        if block_index < 0:
            logger.error(f'<-- The "block_index" should >= 0.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 410, TYPE_FILE, f'The "block_index" should >= 0.', {})) # change 9
            return
    ...
    while True:
        ...
        if request_operation == OP_LOGIN:
            if request_type != TYPE_AUTH:
                ...
                if hashlib.md5(json_data[FIELD_USERNAME].encode()).hexdigest().lower() != json_data[FIELD_PASSWORD].lower(): # change 10
                    connection_socket.send(
...
def tcp_listener(server_ip, server_port):
    ...
    """
    global logger
    server_socket = socket(AF_INET, SOCK_STREAM) # change 11
...
def main():
    ...
    tcp_listener(server_ip, server_port) # change 13
